...
//		}
		...
		*/
		class Student{
			...
			float getAverage(){
				float sum=(int)((((kor+eng+math)/3f)+0.05f)*10)/10f;
				return sum;
			}
...
//				double r=Marh.sqrt((x-x1)*(x-x1)+(y-y1)*(y-y1));
//				return r;
//			}
//		}
//		System.out.println(getDistance(1,1,2,2));
		...
		*/
		...
		b. 생성자는 객체를 생성하기 위한 것이다. -> 객체를 초기화할 때 사용,객체를 생성하는 것은 new연산자
		c. 클래스에는 생성자가 반드시 하나 이상 있어야 한다.
		d. 생성자가 없는 클래스는 컴파일러가 기본 생성자를 추가한다.
		e. 생성자는 오버로딩 할 수 없다. -> 오버로딩 가능. 하나의 클래스에 여러개의 생성자 정의 가능
		*/
		
		/*완료
		[6-11] 다음 중 this에 대한 설명으로 맞지 않은 것은? (모두 고르시오)
		a. 객체 자신을 가리키는 참조변수이다.
		b. 클래스 내에서라면 어디서든 사용할 수 있다. -> 클래스 메서드나 변수에는 사용못함, 인스턴스에서만 사용 가능
		...
		*/
		...
		c. 리턴타입이 달라야 한다. -> 오버로딩에 영향 안줌
		d. 매개변수의 이름이 달라야 한다. -> 오버로딩에 영향 안줌
		
		오버로딩의 조건 
		1.메서드 이름이 같아야함
		2.매개변수의 개수 또는 타입이 달라야 함
		3.매개변수는 같고 리턴타입이 다르면 오버로딩 성립 안됨
		Java의 정석定石 3판 - 연습문제 풀이 39
		*/
		
		/*완료
		[6-13] 다음 중 아래의 add메서드를 올바르게 오버로딩 한 것은? (모두 고르시오)
		long add(int a, int b) { return a+b;} -> 메서드 이름이 같은데 타입까지 같아서 안됨
		...
		*/
		...
		c.초기화 블럭보다 생성자가 먼저 수행된다. -> 초기화 블럭이 먼저 수행된 다음에 수행됨
		d.명시적 초기화를 제일 우선적으로 고려해야 한다.
		e.클래스변수보다 인스턴스변수가 먼저 초기화된다. -> 클래스변수는 클래스가 처음 메모리에 로딩될 때 자동 초기화되기에 먼저 됨
		*/
		
		/*완료
		[6-15] 다음중 인스턴스변수의 초기화 순서가 올바른 것은?
		a. 기본값-명시적초기화-초기화블럭-생성자 -> 정답
		b. 기본값-명시적초기화-생성자-초기화블럭
		c. 기본값-초기화블럭-명시적초기화-생성자
		d. 기본값-초기화블럭-생성자-명시적초기화
		클래스변수 초기화순서 : 기본값-명시적초기화-클래스초기화블럭
		인스턴스변수 초기화순서 : 기본값-명시적초기화-인스턴스초기화블럭-생성자
		*/
		/*정답
		[6-16] 다음 중 지역변수에 대한 설명으로 옳지 않은 것은? (모두 고르시오)
		a. 자동 초기화되므로 별도의 초기화가 필요없다. -> 지역변수는 따로 초기화해야함, call stack에 생성됨
		b. 지역변수가 선언된 메서드가 종료되면 지역변수도 함께 소멸된다.
		c. 매서드의 매개변수로 선언된 변수도 지역변수이다.
		d. 클래스변수나 인스턴스변수보다 메모리 부담이 적다.
		e. 힙(heap)영역에 생성되며 가비지 컬렉터에 의해 소멸된다.->힙에는 인스턴스가 생성됨
		
		지역변수는 자신이 선언된 블럭이나 메서드가 종료되면 소멸됨
		*/
		/*완료
		...
		b. println메서드를 제외한 나머지 메서드들은 모두 종료된 상태이다.->제일 위에 있는 메서드가 수행 중이고 나머지 메서드들은 대기상태
		...
		*/
		...
		static int cv = 20;
		int iv2 = cv;
		static int cv2 = iv; // 라인 A -> static변수의 초기화에 인스턴스변수 사용 못함
										사용하려면 객체 생성
		static void staticMethod1() {
		System.out.println(cv);
		System.out.println(iv); // 라인 B -> 클래스메서드에서 인스턴스변수 사용 못함
		}
		...
		staticMethod1();
		instanceMethod1(); // 라인 D -> 클래스메서드에서 인스턴스메서드 사용 못함
		}
		...
		*/
		...
		change(str);
		System.out.println("After change:"+str); -> ABC123출력, change의 매개변수 str과 
													main()에 있는 str은 다른 변수이기 때문에
			}
		}
		*/
